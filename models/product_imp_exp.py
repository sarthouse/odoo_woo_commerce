    ...
    def export_selected_product_variant(self, instance_id):
        ...
        if list:
            for data in list:
                if data.get('id'):
                    try:
                        parsed_data = wcapi.put("products/%s/variations/%s" % (data.get('name'), data.get('id')), data)
    ...
    def export_selected_product(self, instance_id):
        ...
        if list:
            for data in list:
                if data.get('id'):
                    try:
                        _logger.info('%s', wcapi.put("products/%s" % (data.get('id')), data).json())
                    except Exception as error:
                        raise UserError(_("Please check your connection and try again"))
                else:
                    raise UserError(_("No existen productos actualizables"))
                ...
                if data.content:
                    ...
                    if parsed_data:
                        for ele in parsed_data:
                            ...
                            dict_p['purchase_ok'] = False #if ele.get('purchaseable') and ele.get('purchaseable') == True else True
        ...
        while page > 0 :
            ...
            if data.status_code == 200 and data.content:
                ...
                if len(parsed_data) == 0:
                    page = 0
                for ele in parsed_data:
                    # Buscar el producto por ID de Woocommerce o por SKU de Odoo sin variantes
                    ...
                    if product:
                        ...
                        if stock_quantity is not None and stock_quantity > 0:
                            if product.tracking != 'none':
                                #Si el producto tiene seguimiento por lotes, obtener los lotes existentes en Odoo
                                lots = self.env['stock.lot'].sudo().search([('product_id', '=', product.id)], order='create_date asc')
                                if lots:
                                    # Obtener las cantidades de stock actuales para los lotes existentes
