...
class SaleOrder(models.Model):
    ...
    woo_order_date = fields.Date(string="Fecha del pedido Woo")
    ...
    def import_sale_order(self, instance_id):
        ...
        while page > 0:
            ...
            url = "orders"
            try:
                data = wcapi.get(url, params={'orderby': 'id', 'order': 'asc', 'per_page': 100, 'page': page, 'after': AFTER_ORDERS})
                page += 1
            except Exception as error:
                raise UserError(_("Please check your connection and try again"))
                    ...
                    for ele in parsed_data:
                        ...
                        if not sale_order:
                                ...
                                if ele.get('billing') and ele.get('billing').get('email'):
                                    ...
                                    if email:
                                        ...
                                        if not res_partner:
                                            ...
                                            category_woocommerce = self.env['res.partner.category'].sudo().search([('name', '=', category_name)], limit=1)
                                            if not category_woocommerce:
                                                category_woocommerce = self.env['res.partner.category'].sudo().create({
                                                    'name': category_name
                                                })
                                            if category_woocommerce:
                                                if 'category_id' not in dict_a:
                                                    dict_a['category_id']= [(6,0, [category_woocommerce.id])]
                                                else:
                                                    dict_a['category_id'][0][2].append(category_woocommerce.id)
                                            ...
                                            if ele.get('billing').get('phone'):
                                                dict_a['mobile'] = ele.get('billing').get('phone')
                                            else:
                                                dict_a['mobile'] = ''
                                            ...
                                            if ele.get('billing').get('country'):
                                                ...
                                                for meta in ele.get('meta_data'):
                                                    if meta.get('key') == 'DNI':
                                                        dni_value = meta.get('value')
                                                        if len(dni_value) == 8:
                                                            tipo_id = self.env['l10n_l10n_latam_identification_type_id'].search([('name', '=', 'DNI')], limit=1)
                                                            dict_a['l10n_latam_identification_type_id'] = tipo_id
                                                            dict_a['vat'] = dni_value
                                                            consumidor_final = self.env['l10n_ar.afip.responsibility.type'].search([('name', '=', 'Consumidor Final')], limit=1)
                                                            if consumidor_final:
                                                                dict_a['l10n_ar_afip_responsibility_type_id'] = consumidor_final.id 
                                                        elif len(dni_value) == 11:
                                                            formatted_cuit = '{}{}{}'.format(dni_value[:2], dni_value[2:10], dni_value[10])
                                                            tipo_id = self.env['l10n_l10n_latam_identification_type_id'].search([('name', '=', 'CUIT')],limit=1)
                                                            dict_a['l10n_latam_identification_type_id'] = tipo_id
                                                            dict_a['vat'] = formatted_cuit
                                                            try:
                                                                # Consulta a la API de Tango Factura con el CUIT
                                                                url = f"https://www.tangofactura.com/Rest/GetContribuyente?cuit={formatted_cuit}"
                                                                response = requests.get(url)
                                                                
                                                                if response.status_code == 200:
                                                                    data = response.json()
                                                                    responsabilidad_codigo = data.get('Contribuyente', {}).get('PadronPerfilImpositivoCodigo')
                                                                    
                                                                    # Mapear los códigos recibidos de Tango Factura con los de Odoo
                                                                    responsabilidad_fiscal_map = {
                                                                        'CF': 'Consumidor Final',
                                                                    ...
                                                                    if responsabilidad_codigo in responsabilidad_fiscal_map:
                                                                        # Buscar el tipo de responsabilidad fiscal correspondiente en Odoo
                                                                        responsabilidad_fiscal = self.env['l10n_ar.afip.responsibility.type'].search(
                                                                            [('name', '=', responsabilidad_fiscal_map[responsabilidad_codigo])],
                                                                            limit=1
                                                                        )
                                                                        if responsabilidad_fiscal:
                                                                            dict_a['l10n_ar_afip_responsibility_type_id'] = responsabilidad_fiscal.id
                                                                        else:
                                                                            raise UserError(f"No se encontró el tipo de responsabilidad fiscal en Odoo para el código: {responsabilidad_fiscal_map[responsabilidad_codigo]}")
                                                                    else:
                                                                        raise UserError(f"El código de responsabilidad fiscal '{responsabilidad_codigo}' no es válido o no está mapeado.")
                                                                else:
                                                                    raise UserError("Error en la consulta a la API de Tango Factura.")
                                                            
                                                            except requests.exceptions.RequestException as e:
                                                                raise UserError(f"Error de conexión con la API de Tango Factura: {str(e)}")
                                            ...
                                            if not res_partner:
                                                ...
                                                for meta in ele.get('meta_data'):
                                                    if meta.get('key') == 'DNI':
                                                        ...
                                                        if len(dni_value) == 8: 
                                                            tipo_id = self.env['l10n_l10n_latam_identification_type_id'].search([('name', '=', 'DNI')], limit=1)
                                                            dict_a['l10n_latam_identification_type_id'] = tipo_id  # Asegúrate de que este ID es correcto
                                                            dict_a['vat'] = dni_value
                                                            consumidor_final = self.env['l10n_ar.afip.responsibility.type'].search([('name', '=', 'Consumidor Final')], limit=1)
                                                            if consumidor_final:
                                                                dict_a['l10n_ar_afip_responsibility_type_id'] = consumidor_final.id  # Asegúrate de que este ID es correcto
                                                        elif len(dni_value) == 11:
                                                            formatted_cuit = '{}{}{}'.format(dni_value[:2], dni_value[2:10], dni_value[10])
                                                            tipo_id = self.env['l10n_l10n_latam_identification_type_id'].search([('name', '=', 'CUIT')],limit=1)
                                                            dict_a['l10n_latam_identification_type_id'] = tipo_id  # Asegúrate de que este ID es correcto
                            ...
                            if res_partner:
                                ...
                                url = location + 'wp-admin/admin.php?page=wc-orders&id=' + f"{ele.get('id')}" + '&action=edit'
                                ...
                                for i in ele.get('line_items'):
                                    ...
                                    if res_product:
                                        ...
                                        if i.get('taxes') and len(i['taxes']) > 0:
                                            ...
                                            tax_21_iva = self.env['account.tax'].sudo().search(
                                                [('amount', '=', 21), 
                                                 ('woo_id', '!=', False), 
                                                 ('company_id', '=', instance_id.woo_company_id.id)], limit=1)
                                            if tax_21_iva:
                                                dict_l['tax_id'] = [(6, 0, [tax_21_iva.id])]
                                            else:
                                                dict_l['tax_id'] = [(6, 0, [])]
                                ...
                                if ele.get('payment_method') == 'bacs':
                                    so_obj.payment_type = 'bacs'
                                elif ele.get('payment_method')== 'woo-mercado-pago-basic':
                                    so_obj.payment_type = 'woo-mercado-pago-basic'
                                else:
                                                        ...
                                                        for record in i.get('meta_data'):
                                                            if 'key' in record and record.get('key') == '_vendor_id':
                                                                vendor_id = self.env['res.partner'].sudo().search(
                                                                    [('woo_id', '=', record.get('value'))], limit=1)
                                                                dict_l['woo_vendor'] = vendor_id.id
                                                            # if 'key' in record and record.get('key')== 'afip_pdf':
                                                            #     afip_pdf = self.env['account.move']
                                                    create_p = self.env['sale.order.line'].sudo().create(dict_l)
                                        self.env.cr.commit()
            else:
                page = 0
    ...
    def woo_order_create(self, data):
        ...
        if not sale_order:
            ...
            if res_partner:
                ...
                so_obj.woo_status = ele.get('status')
